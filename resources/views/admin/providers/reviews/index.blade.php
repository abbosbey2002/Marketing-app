                                        ...
                                        <tr>
                                            <td>
                                                <a href="{{ route('reviews.show', $review->id) }}" class="hstack gap-3">
                                                    <div>
                                                        <span class="text-truncate-1-line">{{ $review->user_full_name }}</span>
                                                    </div>
                                                </a>
                                            </td>
                                            <td><a href="#">{{ $review->language->name ?? null }}</a></td>
                                            <td><a href="#">{{ $review->provider->name ?? null }}</a></td>
                                            <td><a href="">{{ $review->created_at }}</a></td>
                                            <td><a href="{{ $review->link }}" target="_blank">{{ __('Link') }}</a></td>
                                            <td>
                                                <div class="hstack gap-2 justify-content-end">
                                                    ...
                                                    <div class="dropdown">
                                                        ...
                                                        <ul class="dropdown-menu">
                                                            <li>
                                                                <a class="dropdown-item" href="javascript:void(0)" data-bs-toggle="offcanvas" data-bs-target="#editReviewProviderOffcanvas{{ $review->id }}" >
                                                                    <i class="feather feather-edit-3 me-3"></i>
                                                                    <span>Edit</span>
                                                                </a>
                                                            </li>
                                                            <li>
                                                                <form class="dropdown-item" action="{{ route('reviews.destroy', $review->id)}}" method="POST" onsubmit="confirmDelete(event)">
                                                                    @csrf
                                                                    @method('DELETE')

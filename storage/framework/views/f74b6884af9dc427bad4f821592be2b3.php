    ...
    <div class="container">
        ...
        <div class="col-lg-6">
            <div class="box-form-register">
                ...
                <form class="form-register" action="<?php echo e(route('providerRegisterStep1')); ?>" method="POST">
                    <?php echo csrf_field(); ?>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="name" id="company-name" class="form-control text-center" placeholder="DORA" required>
                    </div>
                    <div class="form-group">
                        <button class="btn btn-black btn-rounded" type="submit">Next
        ...
        timeout = setTimeout(function() {
            if (query.length > 2) {
                ...
                fetch(`/search-companies?query=${query}`)
                    .then(response => {
                        console.log('Response Status:', response.status); // Javob statusini tekshirish
                        return response.json();
                    })
                    .then(data => {
                        console.log('Response Data:', data); // Javob ma'lumotlarini tekshirish
                        companyList.innerHTML = '';
                        companyList.style.display = 'block';

                        if (data.length) {
                            data.forEach(company => {
                                const companyOption = document.createElement('a');
                                companyOption.href = '#';
                                companyOption.classList.add('list-group-item', 'list-group-item-action', 'company-option');
...
<?php echo $__env->make('layouts.login', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/nazarbek/server/MARKETING/resources/views/auth/provider/register/step1.blade.php ENDPATH**/ ?>